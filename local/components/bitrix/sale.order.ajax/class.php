...
class SaleOrderAjax extends \CBitrixComponent
{
    ...
    public function onPrepareComponentParams($arParams)
    {
        ...
        $arParams['PAY_GIFTCARD'] = $arParams['PAY_GIFTCARD'] === 'Y' ? 'Y' : 'N';
        ...
        $this->arResult = array(
            ...
            'PAYED_FROM_GIFTCARD_FORMATED' => false,
    ...
    protected function initLastOrderData(Order $order)
    {
        ...
        if (
            ...
            if (!empty($lastOrderData)) {
                ...
                if (!empty($lastOrderData['PAY_FROM_GIFTCARD']))
                    $this->arUserResult['PAY_FROM_GIFTCARD'] = $showData['PAY_FROM_GIFTCARD'] = $lastOrderData['PAY_FROM_GIFTCARD'];
    ...
    protected function initProperties(Order $order, $isPersonTypeChanged)
    {
        ...
        foreach ($propertyCollection as $property) {
                // file_put_contents($_SERVER['DOCUMENT_ROOT'].'/local/new_log.txt', print_r([$property->getProperty(),$orderProperties],1),FILE_APPEND);//
            if ($property->isUtil())
                continue;
            ...
            if ($arResult['HAVE_PREPAYMENT'] && !empty($arResult['PREPAY_ORDER_PROPS'][$arProperty['CODE']])) {
                if ($arProperty['TYPE'] === 'LOCATION') {
                    $regionPriceId = regionData::getInfo();

                    if ($regionPriceId["CITY_ID"]) {
                        $res = LocationTable::getList(array(
                            'filter' => array(
                                '=ID' => $regionPriceId["CITY_ID"],
                            ),
            ...
            if ($arProperty['TYPE'] === 'LOCATION') {
                ...
                $change = new userGeolocationCurrent();

                $sessionPositionUser = ['CODE'=>$curVal];

                $sessionPositionUser['LID'] = LANGUAGE_ID;

                $change->setUserLocation( $sessionPositionUser );
            }
...
//			$locationChanged = $locValue != $this->request->get('RECENT_DELIVERY_VALUE');
    ...
    public function generateUserData($userProps = array())
    {
        ...
        //добавляем группу для физ лиц
        $rsGroups = CGroup::GetList ($by = "c_sort", $order = "asc", Array ("STRING_ID" => 'club'));
    ...
    protected function registerAndLogIn($userProps)
    {
		...
		$prop = $this->getPropertyValuesFromRequest();
		if(strlen($userData['NEW_NAME'])<=0)
			$userData['NEW_NAME'] = $prop[2];
        ...
        if (intval($arAuthResult) <= 0) {
            ...
            //$USER->Authorize($arAuthResult);// выключаем авторизацию для физ лиц

            if ($this->arParams['SEND_NEW_USER_NOTIFY'] == 'Y') {
                CUser::SendUserInfo($userId, $this->context->getSite(), Loc::getMessage('INFO_REQ'), true);
            }
    ...
    protected function obtainBasket()
    {
        ...
        foreach ($this->order->getBasket() as $basketItem) {
            ...
            $prodictID = $arBasketItem['PRODUCT_ID'];

            $elementInfo = CIBlockElement::GetByID($prodictID);
            if ($prodictID) {
                if ($elementResult = $elementInfo->GetNext()) {

                    $res = CIBlockSection::GetByID($elementResult["IBLOCK_SECTION_ID"]);
                    if ($arRes = $res->Fetch()) {
                        $arRes['NAME'] = getSectionMessage($arRes['ID'], 'NAME', $arRes['NAME']);
                        $arBasketItem['SECTION'] = sprintf('Коллекция %s', $arRes['NAME']);
                    }
					else
					{
						$db_props = CIBlockElement::GetProperty($elementResult["IBLOCK_ID"], $prodictID, array("sort" => "asc"), Array("CODE"=>"CML2_LINK"));
						if($ar_props = $db_props->Fetch())
						{
							$prodictID_catalog = IntVal($ar_props["VALUE"]);
							$elementInfo_catalog = CIBlockElement::GetByID($prodictID_catalog);
							if ($elementResult_catalog = $elementInfo_catalog->GetNext()) {
								$res_catalog = CIBlockSection::GetByID($elementResult_catalog["IBLOCK_SECTION_ID"]);
								if ($arRes_catalog = $res_catalog->Fetch()) {
									$arRes_catalog['NAME'] = getSectionMessage($arRes_catalog['ID'], 'NAME', $arRes_catalog['NAME']);
									$arBasketItem['SECTION'] = sprintf('Коллекция %s', $arRes_catalog['NAME']);
								}
							}
						}
    ...
    protected function obtainPropertiesForIbElements()
    {
        ...
        foreach ($arResult["BASKET_ITEMS"] as $key => $arBasketItem) {
			...
			if($ar_res = $res->GetNext())
				$arBasketItem["IBLOCK_ID"] = $ar_res["IBLOCK_ID"];
			$db_props = CIBlockElement::GetProperty($arBasketItem["IBLOCK_ID"], $arBasketItem['PRODUCT_ID'], array("sort" => "asc"), Array("CODE"=>"VOLUME"));
			$volume = $db_props->Fetch();
			if (is_array($volume) && count ($volume)>0 && strlen($volume["VALUE"])>0)
			{
				$arBasketItem['PROPERTY_VOLUME_VALUE'] = getVolumeName($volume["VALUE"]);
			}
			if (isset($arBasketItem['PROPERTY_SIZES_VALUE']))
			{
				$arBasketItem['PROPERTY_SIZES_VALUE'] = getSizeName($arBasketItem['PROPERTY_SIZES_VALUE']);
			}
			if (isset($arBasketItem['PROPERTY_COLOR_VALUE']))
			{
				$arBasketItem['PROPERTY_COLOR_VALUE'] = getColorName($arBasketItem['PROPERTY_COLOR_VALUE']);
			}
    ...
    protected function obtainPaySystem()
    {
        ...
        $giftcardPayment = $this->getGiftcardPayment($this->order);
        if (!empty($giftcardPayment) && $giftcardPayment->getSum() > 0) {
            $arResult['PAYED_FROM_GIFTCARD_FORMATED'] = SaleFormatCurrency($giftcardPayment->getSum(), $this->order->getCurrency());
            $arResult['ORDER_TOTAL_LEFT_TO_PAY'] = $this->order->getPrice() - $giftcardPayment->getSum();
            $arResult['ORDER_TOTAL_LEFT_TO_PAY_FORMATED'] = SaleFormatCurrency($this->order->getPrice() - $giftcardPayment->getSum(), $this->order->getCurrency());
        }

        $paySystemList = $this->arPaySystemServiceAll;
        // file_put_contents($_SERVER['DOCUMENT_ROOT'].'/local/new_log.txt', print_r([$paySystemList,$innerPayment],1));//FILE_APPEND
        if (!empty($paySystemList)) {
            ...
            ## выделим отдельно подарочные карты
            foreach ($paySystemList as $paySystem) {
                if($paySystem['CODE'] == "GIFTCARD"){
                    if ($paySystem['LOGOTIP'] > 0) {
                        $paySystem['LOGOTIP'] = CFile::GetFileArray($paySystem['LOGOTIP']);
                    }
                    $arResult['GIFTCARD_PAY_SYSTEM'] = $paySystem;
                    unset($paySystemList[$paySystem['ID']]);
                }
            }
            #####
    ...
    protected function initPersonType(Order $order)
    {
        ...
        // ставим принудительно тип физ лица в зависимости от авторизации
        if ($USER->IsAuthorized()) {
            $arGroups = $USER->GetUserGroupArray();
            $isWholesale = false;

            $rsGroups = CGroup::GetList ($by = "c_sort", $order_by = "asc", Array ("STRING_ID" => 'agents | partners | user_price | yuriki'));
            while($arGroup = $rsGroups->GetNext()){
                if(in_array($arGroup['ID'],$arGroups))
                    $isWholesale = true;
            }
            // $agentGroupId = $rsGroups->Fetch()['ID'];
            /**
             * TODO распределяем с учетом групп
             */
            if($isWholesale){
                // то юрлицо
                $personTypeId = intval($personTypes[2]["ID"]);
                $order->setPersonTypeId($personTypeId);
                $this->arUserResult['PERSON_TYPE_ID'] = $personTypeId;
                $personTypes[2]["CHECKED"] = "Y";
                $arResult["PERSON_TYPE"][2] = $personTypes[2];
                //$arResult["PERSON_TYPE"][1] = $personTypes[1]; // без возможности смены
            }
            else {
                // то физлицо
                $personTypeId = intval($personTypes[1]["ID"]);
                $order->setPersonTypeId($personTypeId);
                $this->arUserResult['PERSON_TYPE_ID'] = $personTypeId;
                $personTypes[1]["CHECKED"] = "Y";
                $arResult["PERSON_TYPE"][1] = $personTypes[1];
                //$arResult["PERSON_TYPE"][2] = $personTypes[2]; // без возможности смены
            }
    ...
    protected function loadUserGiftCards(Order $order)
    {
        if (!isset($this->arResult["USER_GIFTCARDS"])) {
            // получим все подарочные карты для пользователя
            $card = new CGiftCards($order->getUserId());
            $arCards = $card->getList();
            $arCardsTotal = $card->getTotalSumm();

            $this->arResult["USER_GIFTCARDS"] = $arCards;
            $this->arResult["USER_GIFTCARDS_TOTAL"] = $arCardsTotal;
        }
    }
    ...
    protected function getInnerPaySystemInfo(Order $order, $recalculate = false)
    {
        ...
        if ($this->arParams['PAY_FROM_ACCOUNT'] === 'Y' && $order->isAllowPay()) {
            ...
            // добавим обработку свойства с суммой для списания
            $arProps = $order->getPropertyCollection()->getArray();
            if(!empty($arProps['properties'])){
                foreach ($arProps['properties'] as $key => $prop) {
                    if($prop['CODE'] == 'SPEND_BONUSES'){
                        if($this->getPropertyValuesFromRequest()[$prop['ID']] > 0 && $this->getPropertyValuesFromRequest()[$prop['ID']] <= $userBudget){
                            $userBudget = $this->getPropertyValuesFromRequest()[$prop['ID']];
                        }
                    }
                }
            }
            // file_put_contents($_SERVER['DOCUMENT_ROOT'].'/local/debug_order.log',print_r($arProps['properties'],1));
            if($innerPayment){
                ...
                if (!empty($sumRange)) {
					...
					//тут подшаманим
					if (!empty($sumRange['MAX'])){
						$range = $order->getPrice()/$sumRange['MAX'];
						$discount = $basePrice - $order->getPrice();
						//проверим корзину на присутствие в ней образцов
						//все образцы вычитаем из суммы
						if(!empty($this->arParams['EXCLUDE_INTERNAL_PAYMENTS_IBLOCK'])){
							foreach($basket as $basket_item){
								$arItem = CIBlockElement::getById($basket_item->getProductId())->Fetch();
								if(in_array($arItem['IBLOCK_ID'],$this->arParams['EXCLUDE_INTERNAL_PAYMENTS_IBLOCK'])){
									$basePrice = $basePrice - $basket_item->getFinalPrice();
								}
							}
						}
						$sumRange['MAX'] = $basePrice/$range;
						//уберем скидку из суммы на списание
						if($discount > 0){
							$sumRange['MAX'] = $basePrice/$range - $discount;
						}
					}

                    if (isset($sumRange['MAX']) && $sumRange['MAX'] <= $userBudget) {
                        $sumToSpend = floatval($sumRange['MAX']);
                    }
                } else {
                    $sumToSpend = $userBudget;
                }
    ...
    public function getGiftcardPayment(Order $order)
    {
        /** @var Payment $payment */
        foreach ($order->getPaymentCollection() as $payment) {
            if (
                ($payment->getPaySystem())->getField('CODE') == 'GIFTCARD'
            ){
                return $payment;
            }
        }
    ...
    protected function createGiftcardPayment(Order $order){
        ...
        if($paySystem = $paySystemResult->fetch())
        {
            $payment = ($order->getPaymentCollection())->createItem(\Bitrix\Sale\PaySystem\Manager::getObjectById($paySystem['PAY_SYSTEM_ID']));
     ...
     * @throws Main\ObjectNotFoundException
     */
    protected function getGiftcardPaySystemInfo(Order $order, $recalculate = false)
    {
        $arResult =& $this->arResult;
        ...
        if ($this->arParams['PAY_GIFTCARD'] == 'Y' && $order->isAllowPay()) {
            $giftcardPaySystemId = 0;
            $giftcardPayment = null;
            $paymentCollection = $order->getPaymentCollection();
            foreach ($paymentCollection as $payment) {
                $ps = $payment->getPaySystem();
                if($ps->getField('CODE') == 'GIFTCARD'){
                    $giftcardPayment = $payment;
                    $giftcardPaySystemId = $payment->getPaymentSystemId();
                }
            }

			if (!$giftcardPayment)
			{
				$giftcardPayment = $this->createGiftcardPayment($order);
                $giftcardPaySystemId = $giftcardPayment->getPaymentSystemId();
			}
            ...
            $this->loadUserGiftCards($order);
            $giftcardBudget = $this->arResult['USER_GIFTCARDS_TOTAL'];//0;//
            ...
            if(!empty($arProps['properties'])){
                foreach ($arProps['properties'] as $key => $prop) {
                    if($prop['CODE'] == 'SPEND_GIFTCARD'){
                        if($this->getPropertyValuesFromRequest()[$prop['ID']] > 0){
                            $giftcardBudget = $this->getPropertyValuesFromRequest()[$prop['ID']];
                        }
                    }
                }
            }
            ...
            if($giftcardPayment){
				$basket = $order->getBasket();
				$basePrice = $basket->getBasePrice();
                $sumRange = Sale\Services\PaySystem\Restrictions\Manager::getPriceRange($giftcardPayment, $giftcardPaySystemId);

                if (!empty($sumRange)) {
                    if (
                        (empty($sumRange['MIN']) || $sumRange['MIN'] <= $giftcardBudget)
                        && (empty($sumRange['MAX']) || $sumRange['MAX'] >= $giftcardBudget)
                    ) {
                        $sumToSpend = $giftcardBudget;
                    }
                    ...
                    if (!empty($sumRange['MAX']) && $sumRange['MAX'] <= $giftcardBudget) {
                ...
                $sumToSpend = $sumToSpend >= $order->getPrice() ? $order->getPrice() : $sumToSpend;
                if (!empty($this->arResult['USER_GIFTCARDS']) && $sumToSpend > 0) {
                    // setting inner payment price
                    // $giftcardPayment->setField('SUM', $sumToSpend);
                    // getting allowed pay systems by restrictions
                    $arPaySystemServices = PaySystem\Manager::getListWithRestrictions($giftcardPayment);
                    // delete inner pay system if restrictions has not passed
                    if (!isset($arPaySystemServices[$giftcardPaySystemId])) {
        ...
        if ($sumToSpend > 0) {
            $arResult['PAY_GIFTCARD'] = 'Y';
            $arResult['GIFTCARD_BUDGET_FORMATED'] = SaleFormatCurrency($arResult['USER_GIFTCARDS_TOTAL'], $order->getCurrency());
        } else {
            $arResult['PAY_GIFTCARD'] = 'N';
            unset($arResult['GIFTCARD_BUDGET_FORMATED']);
        }
    ...
    public function getExternalPayment(Order $order)
    {
        /** @var Payment $payment */
        foreach ($order->getPaymentCollection() as $payment) {
            if ($payment->getPaymentSystemId() != PaySystem\Manager::getInnerPaySystemId() && ($payment->getPaySystem())->getField('CODE') != 'GIFTCARD')
                return $payment;
        }
    ...
    protected function initPayment(Order $order)
    {
        ...
        list($sumToSpend, $giftcardPaySystemList) = $this->getGiftcardPaySystemInfo($order, true);

        if ($sumToSpend > 0) {
            $giftcardPayment = $this->getGiftcardPayment($order);
            if (!empty($giftcardPayment)) {
                if ($this->arUserResult['PAY_FROM_GIFTCARD'] === 'Y') {
                    $giftcardPayment->setField('SUM', $sumToSpend);
                } else {
                    $giftcardPayment->delete();
                    $giftcardPayment = null;
                ...
                $this->arPaySystemServiceAll = $this->arActivePaySystems = $giftcardPaySystemList;
            }
        }
    ...
    protected function recalculatePayment(Order $order)
    {
        ...
        if (!empty($giftcardPayment))
            $remainingSum = empty($giftcardPayment) ? $order->getPrice() : $order->getPrice() - $giftcardPayment->getSum();
        if ($remainingSum > 0 || $order->getPrice() == 0) {
            $paymentCollection = $order->getPaymentCollection();
            ...
            if (empty($giftcardPaySystemList[$giftcardPaySystemId])) {
                unset($extPaySystemList[$giftcardPaySystemId]);
            } elseif (empty($extPaySystemList[$giftcardPaySystemId])) {
                $extPaySystemList[$giftcardPaySystemId] = $giftcardPaySystemList[$giftcardPaySystemId];
            }
    ...
    /**
     * проверяем, применялась ли оплата с внутреннего счета и удаляем / применяем купон
     */
    protected function checkCouponList(){
        global $USER;
        $order = $this->getOrder($USER->GetID() ? $USER->GetID() : CSaleUser::GetAnonymousUserID());
        $arCoupons = DiscountCouponsManager::get(true, array(), true, true);
        // file_put_contents($_SERVER['DOCUMENT_ROOT'].'/local/debug_order.log',print_r($order->getPropertyCollection()->getArray(),1));
        if ($this->request->get('PAY_CURRENT_ACCOUNT') == "Y" || $this->request->get('PAY_FROM_GIFTCARD') == "Y"){
            if (!empty($arCoupons)) {
            ...
            $couponPropValue = '';
            $arr = $order->getPropertyCollection()->getArray();
            foreach($arr['properties'] as $prop){
                if($prop['CODE'] == 'BONUS_CODE' && $prop['ACTIVE'] == 'Y' && !empty($prop['VALUE'][0]))
                    $couponPropValue = $prop['VALUE'][0];
            }

            if(!empty($couponPropValue)){
                DiscountCouponsManager::add($couponPropValue);
            }
        }
    }
...
$arShip = array();
$arPrivate = array(3=>[71,66],70=>[67],71=>[66]);
			foreach ($this->arDeliveryServiceAll as $key => $deliveryObj)
			{
				if ($deliveryObj->getId()==3)
				{
					//unset($this->arDeliveryServiceAll[$key]);
						//continue;
				}
				if(isset($arPrivate[$deliveryObj->getId()]))
				{
					$skip=false;
					foreach ($arPrivate[$deliveryObj->getId()] as $val)
					{
						if(isset($arShip[$val]))
							$skip=true;
					}
    ...
    protected function saveOrderAjaxAction()
    {
        ...
        if ($this->checkSession) {
            ...
            if (empty($this->arResult["ERROR"])) {
                $arOrderRes["REDIRECT_URL"] = $this->arParams["~CURRENT_PAGE"] . "?ORDER_ID=" . urlencode($this->arResult["ACCOUNT_NUMBER"]) . ($fizlico ? '&fizlico=' . $fizlico : '');
    ...
    protected function getJsDataResult()
    {
        ...
        foreach ($arResult["PAY_SYSTEM"] as $id => $paysystem) {
            $arResult["PAY_SYSTEM"][$id]['NAME'] = getParamMessage('order_PAY_SYSTEM_' . $paysystem['PAY_SYSTEM_ID'], 'PARAMETER', $paysystem['NAME']);
        }
        $result['PAY_SYSTEM'] = $arResult["PAY_SYSTEM"];
        $result['INNER_PAY_SYSTEM'] = $arResult["INNER_PAY_SYSTEM"];
        $result['GIFTCARD_PAY_SYSTEM'] = $arResult['GIFTCARD_PAY_SYSTEM'];

       /*foreach ($arResult["DELIVERY"] as $id => $delivery) {
		   if($id==66 || $id==67)
            $arResult["DELIVERY"][$id]['NAME'] = getParamMessage('order_DELIVERY_' . $delivery['ID'], 'PARAMETER', $delivery['NAME']);
        }*/
        $result['DELIVERY'] = $arResult["DELIVERY"];

        $result['MESSAGES'] = array();
        $result['MESSAGES']['order_art'] = getParamMessage('order_art', 'PARAMETER', "Артикул");
        $result['MESSAGES']['order_razmer'] = getParamMessage('order_razmer', 'PARAMETER', "Размер");
		$result['MESSAGES']['order_volume'] = getParamMessage('order_volume', 'PARAMETER', "Объем");
		$result['MESSAGES']['order_color'] = getParamMessage('order_color', 'PARAMETER', "Цвет");
        $result['MESSAGES']['order_RUB'] = getParamMessage('order_RUB', 'PARAMETER', " руб.");
        $result['MESSAGES']['order_pickup_punkt'] = getParamMessage('order_pickup_punkt', 'PARAMETER', 'Пункт самовывоза');
        $result['MESSAGES']['order_adress'] = getParamMessage('order_adress', 'PARAMETER', 'Адрес доставки');
        $result['MESSAGES']['order_DELIV_TYPE'] = getParamMessage('order_DELIV_TYPE', 'PARAMETER', 'Способы доставки');
        ...
        foreach ($arr['properties'] as $key => $property) {
            if ($property['UTIL'] === 'Y'
                ...
                $arr['properties'][$key]['NAME'] = getParamMessage('order_property_' . $property['ID'], 'PARAMETER', $property['NAME']);
            }
        ...
        $result['PAY_FROM_GIFTCARD'] = 'N';
        if ($giftcardPaySystem = $this->getGiftcardPayment($this->order)) {
            if ($giftcardPaySystem->getSum() > 0) {
                $result['PAY_FROM_GIFTCARD'] = 'Y';
            }
        }

        $result['PAY_FROM_ACCOUNT'] = $arResult["PAY_FROM_ACCOUNT"];
        $result['PAY_GIFTCARD'] = $this->arParams["PAY_GIFTCARD"];
        $result['CURRENT_BUDGET_FORMATED'] = $arResult["CURRENT_BUDGET_FORMATED"];
        $result['GIFTCARD_BUDGET_FORMATED'] = $arResult["GIFTCARD_BUDGET_FORMATED"];
        ...
        $result['TOTAL'] = array(
            ...
            'PAYED_FROM_GIFTCARD_FORMATED' => $arResult["PAYED_FROM_GIFTCARD_FORMATED"],
    ...
    public static function scaleImages(&$result, $scale = '')
    {
        if (!empty($result) && is_array($result)) {
            ...
            if (!empty($result['GIFTCARD_PAY_SYSTEM']) && is_array($result['GIFTCARD_PAY_SYSTEM']) && !empty($result['GIFTCARD_PAY_SYSTEM']["LOGOTIP"])) {
                self::resizeImage($result['GIFTCARD_PAY_SYSTEM'], 'LOGOTIP', $result['GIFTCARD_PAY_SYSTEM']["LOGOTIP"],
                    array("width" => 600, "height" => 600),
                    array("width" => 95, "height" => 55),
                    $scale
                );
    ...
    protected function makeUserResultArray()
    {
        ...
        if ($request->isPost()) {
            ...
            if (empty($arUserResult["PERSON_TYPE_OLD"]) || $arUserResult["PERSON_TYPE_OLD"] == $arUserResult["PERSON_TYPE_ID"]) {
                ...
                if ($request->get('PAY_FROM_GIFTCARD') == "Y")
                    $arUserResult["PAY_FROM_GIFTCARD"] = "Y";
    ...
    protected function showOrderAction()
    {
        ...
        $userId = $USER->GetID();
        if (!empty($this->request->get('fizlico'))) {
            $userId = $this->request->get('fizlico');
        }else if ($access = $this->request->get('access'))
		{
			if (
				$order &&
				$order->getHash() === $this->request->get('access') &&
        ...
        if (!empty($arOrder) && ($order->getUserId() == $userId || $checkedBySession)) {
