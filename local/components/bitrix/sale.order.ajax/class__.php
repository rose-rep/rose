...
//			$locationChanged = $locValue != $this->request->get('RECENT_DELIVERY_VALUE');
    ...
    protected function initPersonType(Order $order)
    {
        ...
        if ($USER->IsAuthorized()) {
            ...
            $rsGroups = CGroup::GetList ($by = "c_sort", $order_by = "asc", Array ("STRING_ID" => 'agents | partners | user_price'));
    ...
    public function getInnerPayment(Order $order)
    {
        ...
        foreach ($order->getPaymentCollection() as $payment) {
            file_put_contents($_SERVER['DOCUMENT_ROOT'].'/local/debug_order.log',print_r([$payment->getPaymentSystemId(),PaySystem\Manager::getInnerPaySystemId()],1),FILE_APPEND);
    ...
    protected function checkCouponList(){
        ...
        if ($this->request->get('PAY_CURRENT_ACCOUNT') == "Y"){
            if (!empty($arCoupons)) {
